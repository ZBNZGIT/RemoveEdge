...
powershell -Command "$url = 'https://raw.githubusercontent.com/ZBNZGIT/RemoveEdge/main/setup.exe'; $path = '%tmp%\setup.exe'; try { (New-Object Net.WebClient).DownloadFile($url, $path) } catch { Write-Host 'Error downloading the file.' }"
